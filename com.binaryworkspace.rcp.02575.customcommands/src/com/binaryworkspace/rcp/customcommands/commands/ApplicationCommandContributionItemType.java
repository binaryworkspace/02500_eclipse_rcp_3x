...
/**
 * A convenience enumeration that can be used to access Custom (application
 * specific) CommandContributionItems that can be used to contribute to menus
 * and toolbars.
 * 
 ...
 * 
 */
public enum ApplicationCommandContributionItemType {
	BACK("", ApplicationCommandType.BACK.getCommand().getId(), SWT.PUSH, "Back", PlatformUI.getWorkbench().getSharedImages().getImageDescriptor(ISharedImages.IMG_TOOL_BACK)), //
	FORWARD("", ApplicationCommandType.FORWARD.getCommand().getId(), SWT.PUSH, "Forward", PlatformUI.getWorkbench().getSharedImages().getImageDescriptor(ISharedImages.IMG_TOOL_FORWARD)); //
	...
	private ApplicationCommandContributionItemType(String id, String commandId, int style, String label, ImageDescriptor imageDescriptor) {
		this.id = id;
